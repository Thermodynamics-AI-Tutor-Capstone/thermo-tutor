...
| [**bioTutor**](biotutor-eth.md) `[read]` | ETH Zurich | Biology | ⭐ **Open source, and the system prompt is published.** The UI *forces* the student to state their own answer before it responds — productive failure as a turn-taking rule. 23 weeks, 10,465 interactions. ⚠ **30% uptake, n = 73 for every satisfaction number, no learning outcome, and nobody audited whether it followed its own design** |
