...
9. ⭐⭐ **The two strongest positive results in this literature are both *hybrid*, and neither is
   ...
   supervised every message**, with the remit to revise each draft *"until they would be satisfied
   sending it themselves."* They accepted **74.4% unedited**, and a full audit of 3,617 drafts
   found **zero harmful messages and five factual errors (0.1%)**.

   **LearnLM never spoke to a student unsupervised.** So the headline number everyone cites for
   "AI tutoring works" is really *AI drafting for a human who approves*. **Nobody has a comparably
...
10. ⭐ **The persona may be a liability.** Anderson and Koedinger's 1995 review of a decade of
    ...
    **short and directed error messages**."* Thirty years later, the single largest complaint
    against [LearnLM in Google's own arena](systems/learnlm.md) was **conversational style —
    "patronizing," 36.3%** — and [UIC students preferred rivals for being more succinct](evidence/student-ai-perceptions-2025.md).
    **Two independent lines, three decades apart, say the warm conversational persona is not the
    win it is assumed to be.** Worth testing rather than inheriting.
    → [Cognitive Tutor](systems/cognitive-tutor.md)

11. **Students route around Socratic design.** In 2,874 coded student turns with a
...
12. **Every serious system independently converged on the same architecture:** a
...
13. ⚠ **Provenance buys accuracy, not depth — and we had been conflating them.** Across **seven
    ...
    be adopted by real recipients**. Critically this held *"**even when the results were augmented
    by real-time web links**."* **Showing sources did not restore depth.**

    Every grounded tutor here treats source display as the trust answer. It is — for correctness.
    **Being told something well is a shallower act than assembling it yourself**, and that needs
    [step-based interaction](concepts/vanlehn-2011.md) and
    [productive failure](concepts/productive-failure.md), not better citations. It is the same
...
14. **Grounding + verification is what separates working systems from demos.**
...
15. **Cost is not the constraint.** KAIST ran a 14-week tutor for **477 students on $180
...
16. **No LLM can reliably tell a wrong step from a right one.** Across 223 tutoring domains,
...
17. **The thermodynamics diagram gap is large but model-specific.** Mean accuracy on
...
18. **The benchmark we were going to build already exists.** [ThermoQA](domain/thermoqa.md)
...
19. ⚠ **Stan is not the competitor we thought.** Read in full, the Delaware thermodynamics
...
20. ⚠ **The obvious success metric will train the tutor to cheat.** Across 10,235 submissions,
...
21. ⭐ **Our exact experiment was run in 2006, and the missing piece is the piece we have.**
